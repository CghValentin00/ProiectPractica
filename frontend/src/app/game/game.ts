...
export class Game implements OnInit {
  ...
  constructor(
    ...
    readonly scoresService: ScoresService
  ) {
  ...
  drawSnake(): void {
    ...
    if (!grid) return;
    const segments = grid.querySelectorAll('.snake-segment');
  ...
  loadHighScores(): void {
    ...
    console.log('High scores loaded:', this.highScores.map(s => `${s.username}: ${s.score}`));
  }
